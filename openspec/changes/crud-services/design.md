...
`DELETE` (soft), and `GET /{id}` — and extend the existing
...
**Identifier decision (locked with the user)**: write paths and the `GET /{id}`
endpoint identify resources by `id` (UUID, generated by JPA). `code` is a
human-readable unique business key, exposed in response bodies, but not used in
URLs. Rationale: the front-end sends UUIDs in CRUD operations.
...
ServiceController  ──@RestController──▶  CreateServiceUseCase / UpdateServiceUseCase
   (userinterfaces)                       DeleteServiceUseCase / GetServiceByIdUseCase
                                              │
                                              ▼
...
`description` (100), `enabled` (default `true`), and Envers `@Audited`. The
`id` is the only identifier used by the new write endpoints.
...
`findById(String id)` is already provided by `JpaRepository<Services, String>` and
is the primary lookup for write paths and the new GET endpoint.
...
```java
Optional<Services> findById(String id);                  // CHANGED: was ServiceResourceDto, now entity+Optional
List<ServiceResourceDto> findAll();
Services save(Services entity);                          // NEW: covers create + update (JPA upsert)
Optional<Services> findEnabledById(String id);           // NEW: explicit "not soft-deleted" lookup by id
void disableById(String id);                             // NEW: soft delete at persistence layer
```

**Contract change**: `findById` returns the **entity** (not a DTO) wrapped in
...
- `Services entity = mapper.toServices(trimmedCode, trimmedDescription);`
- `return mapper.toResource(port.save(entity));`

**`UpdateServiceUseCase`**
- Input: `String id` (path), `UpdateServiceRequest`
- Flow: `String trimmedCode = request.code().trim(); String trimmedDescription = request.description().trim();`
- `Services current = port.findById(id).orElseThrow(() -> new NotFoundException("Service", id));`
...
- Input: `String id`
- Flow: `if (port.findEnabledById(id).isEmpty()) throw new NotFoundException("Service", id);`
- `port.disableById(id);` — adapter loads the entity, sets `enabled=false`, persists.
- No return value (`void`); controller responds `204 No Content`.

**`GetServiceByIdUseCase`**
- Input: `String id`
- Flow: `return port.findById(id).map(mapper::toResource).orElseThrow(() -> new NotFoundException("Service", id));`
...
| `PATCH` | `/api/v1/services/{id}` | `200 OK` | `UpdateServiceRequest` (`@Valid`) | `ServiceResourceDto` |
| `DELETE` | `/api/v1/services/{id}` | `204 No Content` | — | — |
| `GET` | `/api/v1/services/{id}` | `200 OK` | — | `ServiceResourceDto` |
...
| `GET/PATCH/DELETE /{id}` miss | `NotFoundException("Service",id)` | 404 | 404 | `Not Found` | `Resource not found` | `[ex.getMessage()]` |
...
### 5.2 Error path — `PATCH /api/v1/services/{id}` with duplicate description

```
Client
  │  PATCH { "code":"SRV-001", "description":"  Checkup  " }     // "Checkup" already used by another service
  ▼
ServiceController.update(id, req)
  │  @Valid passes (both non-blank, within size)
  ▼
UpdateServiceUseCase.update(id, req)
  │  trimmedCode = "SRV-001", trimmedDesc = "Checkup"
  │  port.findById(id)                 → Optional[Services(id=id, code="SRV-001", description="Old", enabled=true)]
  │  code unchanged  → skip existsByCodeAndIdNot
  │  desc changed    → port.existsByDescriptionAndIdNot("Checkup", id) → true
  │  throw new ExistsException("description","Service","Checkup")
  ▼
GlobalExceptionHandler.handleExistsException(...)
  │  body = ErrorResponse(CONFLICT, "Resource already exists", "uri=/api/v1/services/<id>")
...
| `service/application/service/UpdateServiceUseCaseTest` | same-row self-exclusion (sending its own `code`/`description` is a no-op for uniqueness); `ExistsException` on cross-row conflict; `NotFoundException` on `findById` empty; `setCode`/`setDescription` invoked with trimmed values; `port.save(current)` called once. |
| `service/application/service/DeleteServiceUseCaseTest` | `port.findEnabledById(id)` empty → `NotFoundException`; present → `port.disableById(id)` invoked once. |
| `service/application/service/GetServiceByIdUseCaseTest` | present → returns mapped DTO; empty → `NotFoundException`. |
...
| update happy | `PATCH` | `/api/v1/services/{id}` | 200 |
| update not found | `PATCH` | `/api/v1/services/{id}` | 404 |
| update duplicate other | `PATCH` | (cross-row) | 409 |
| update blank code | `PATCH` | empty `code` | 400 + `errors` |
| delete happy | `DELETE` | `/api/v1/services/{id}` | 204 (empty body) |
| delete not found | `DELETE` | `/api/v1/services/{id}` | 404 |
| get happy | `GET` | `/api/v1/services/{id}` | 200 |
| get not found | `GET` | `/api/v1/services/{id}` | 404 |
...
3. **No prior global handler** — every controller in every context was 500-ing
   ...
   same change.
4. **`findById` port contract change** (nullable → `Optional<Services>`) —
   ...
   `servicesPersistencePort.findById` and `ServiceServicePort.findById`
   during the change.
...
- `GET /api/v1/services/{code}` — explicit non-goal; paths use `id` (UUID) per
  user decision.
