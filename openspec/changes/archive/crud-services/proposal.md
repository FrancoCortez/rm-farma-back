...
## Why
The `service` bounded context (`owl.tree.rmfarma.service`) currently exposes only `GET /api/v1/services`. Operators cannot create, update, or deactivate a service through the API, and there is no global error handler — so any future validation failure would surface as a raw 500. This change closes that gap by adding the missing CRUD endpoints (POST, PATCH, soft DELETE, GET by id) and introducing a reusable `GlobalExceptionHandler` in the `shared` module so that uniqueness violations, not-found misses, and bean-validation errors translate into proper 409/404/400 responses with a uniform shape.
...
- **[NEW]** `DeleteServiceUseCase` (soft) — loads by id, sets `enabled = false`, persists.
- **[NEW]** `GetServiceByIdUseCase` — finds by id, throws `NotFoundException` on miss.
...
- **[MODIFY]** `ServiceServicePort` (in port): add `create(CreateServiceRequest)`, `update(String id, UpdateServiceRequest)`, `deleteById(String id)`, `findById(String id)`.
- **[MODIFY]** `ServicesPersistencePort` (out port): add `save(Services)`, `update(Services)`, `deleteById(String id)`, change `findById(String)` to return `Optional<Services>` instead of nullable entity.
- **[MODIFY]** `ServicesPersistencePortAdapter`: implement new port methods; change `findById` to return `Optional<Services>` (empty on miss — the use case layer is the one that translates to `NotFoundException`, keeping the persistence adapter pure).
...
- **[MODIFY]** `ServiceController`: add `POST /api/v1/services`, `PATCH /api/v1/services/{id}`, `DELETE /api/v1/services/{id}` (soft), `GET /api/v1/services/{id}`. Existing `GET /api/v1/services` stays untouched.
...
- `services` — the bounded context's public contract expands with create, update, soft delete, and find-by-id.
...
- **Affected APIs**: 4 new endpoints under `/api/v1/services/*` (`POST`, `PATCH`, `DELETE`, `GET /{id}`).
- **Data migration**: none.
- **Backward compatibility**: `GET /api/v1/services` is unchanged. Existing `Services` entity is not modified in shape (only new repository methods). The `findById` port contract changes from nullable to `Optional`, which is safe today because no production caller consumes it.
...
- [ ] `ServicesPersistencePortAdapter.findById` returning null today: changing it to `Optional` is a contract change. Today only the unused port is affected; verify no production caller exists before merging.
...
## Open Questions
- (None — user decisions locked: uniqueness app-only, error handler in same change, CRUD paths use id not code — front sends UUIDs).
